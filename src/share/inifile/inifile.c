                      ...
                      int *key_s,int *key_e, int *value_s, int *value_e)
{
    const char *p = buf;
    int i=0;
    int sectionlen2=strlen(section);
    assert(buf!=NULL);
    assert(section != NULL && sectionlen2);
    assert(key != NULL && strlen(key));
    int nComp=strlen(key);
    *sec_e = *sec_s = *key_e = *key_s = *value_s = *value_e = -1;
    ...
    while( !end_of_string(p[i]) ) {
        ...
        if( ( 0==i ||  newline(p[i-1]) ) && left_barce(p[i]) )
        {
            ...
            int sectionlen=i-section_start;//lcp
            if( sectionlen2==sectionlen && 0 == strncmp(p+section_start,section, sectionlen)) {
                int newline_start=0;
                ...
                while( ! (newline(p[i-1]) && left_barce(p[i]))
                    ...
                    if(';' != p[j]) //skip over comment
                    {
                        while(j < i && p[j]!='=') {
                            ...
                            } else if('=' == p[j] && nComp==(j-newline_start-1)) {//= 带空格 " = "
...
int64_t read_profile_int64( const char *section, const char *key,int64_t default_value,const char *file)
{
    char value[32] = {0};
    if(!read_profile_string(section,key,value, sizeof(value),NULL,file))
    {
                         ...
                         const char *value, const char *file)
{
    ...
    if( -1 == sec_s)
    {
        ...
        else
        {
            ...
            if(file_size > (MAX_FILE_SIZE) )  {printf("ini file,long file_size:%d\n,",file_size); file_size=MAX_FILE_SIZE;}
    ...
    else if(-1 == key_s)
    {
        ...
        if(sec_e > (MAX_FILE_SIZE) )  {printf("ini file,long sec_e:%d\n,",sec_e); sec_e=MAX_FILE_SIZE;}
