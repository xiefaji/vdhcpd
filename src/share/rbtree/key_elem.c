...
struct key_node *key_rbinsert(struct key_tree *tree,const key_type first,void *second)
{
    ...
    struct key_node *node_t=(struct key_node *)RBMALLOC(sizeof(struct key_node));
...
struct key_node *Rbtree_FindLastNode_FollowKey(struct key_tree *tree,const unsigned int key)
{
    struct rb_root *root = &tree->root;
