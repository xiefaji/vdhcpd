...
PUBLIC int packet_do_dpi(packet_process_t *packet_process)
{
    ...
    // 设定DHCP类型[V4/V6]
    if (ETH_P_IP == protocoltype)
        process = DEFAULT_DHCPv4_PROCESS;
    else if (ETH_P_IPV6 == protocoltype)
        process = DEFAULT_DHCPv6_PROCESS;
...
PRIVATE int packet_parse(packet_process_t *packet_process)
{
    ...
    case DEFAULT_DHCPv6_PROCESS: {
        ...
        if (l3len < request->l3len || l4len < request->l4len || iphdr_len < sizeof(struct ip6_hdr) || ((l4len + iphdr_len) != l3len)) {
            x_log_debug("%s:%d 错误的报文[%u] l3len[%u/%u] l4len[%u/%u] iphdr_len[%u]", __FUNCTION__, __LINE__, DEFAULT_DHCPv6_PROCESS,
                        l3len, request->l3len, l4len, request->l4len, iphdr_len);
            retcode = -1;
        }
    } break;
    default:
        retcode = -1;
        x_log_debug("%s:%d 未识别Processl数据类型[%u][%u].", __FUNCTION__, __LINE__, packet_process->dpi.process, errno);
        break;
    }
...
PRIVATE int packet_deepin_parse4(packet_process_t *packet_process, trash_queue_t *pRecycleTrash)
{
    ...
    if (request->payload_len < offsetof(struct dhcpv4_message, options) + 4 || req->op != DHCPV4_BOOTREQUEST || req->hlen != ETH_ALEN) {
        x_log_debug("v4报文头解析失败");
        return -1;
    }
    ...
    if (request->v4.msgcode == DHCPV4_MSG_DISCOVER) {
        BZERO(&realtime_info->v4, sizeof(realtime_info->v4));
        SET_COUNTER(realtime_info->starttick);
    }
    if (hostname_len) {
        BCOPY(hostname, realtime_info->v4.hostname, MAXNAMELEN);
        realtime_info->v4.hostname_len = hostname_len;
    }
    if (reqopts_len) {
        BCOPY(reqopts, realtime_info->v4.reqopts, MAXNAMELEN);
        realtime_info->v4.reqopts_len = reqopts_len;
    }
    if (vendorname_len) {
        BCOPY(vendorname, realtime_info->v4.vendorname, MAXNAMELEN);
        realtime_info->v4.vendorname_len = vendorname_len;
    }
    if (clientidentifier_len) {
        BCOPY(clientidentifier, realtime_info->v4.clientidentifier, MAXNAMELEN);
        realtime_info->v4.clientidentifier_len = clientidentifier_len;
    }
    if (userclass_len) {
        BCOPY(userclass, realtime_info->v4.userclass, MAXNAMELEN);
        realtime_info->v4.userclass_len = userclass_len;
    }
...
PRIVATE int packet_process4(packet_process_t *packet_process, trash_queue_t *pRecycleTrash)
{
    ...
    if (dhcpd_server_match_macaddr(dhcpd_server, packet_process->macaddr))
        return -1;
#ifdef CLIB_DEBUG
    x_log_debug("v4服务器:%d", ENABLE_IPV4_SERVER(dhcpd_server));
#endif // DEBUG
    // 报文分业务处理
...
PRIVATE int packet_deepin_parse6(packet_process_t *packet_process, trash_queue_t *pRecycleTrash)
{
    ...
    if (request->v6.msgcode == DHCPV6_MSG_SOLICIT) {
        BZERO(&realtime_info->v6, sizeof(realtime_info->v6));
        SET_COUNTER(realtime_info->starttick);
    }
    if (duid_len) {
        BCOPY(duid, realtime_info->v6.duid, MAXNAMELEN);
        realtime_info->v6.duid_len = duid_len;
    }
    if (hostname_len) {
        BCOPY(hostname, realtime_info->v6.hostname, MAXNAMELEN);
        realtime_info->v6.hostname_len = hostname_len;
    }
    if (reqopts_len) {
        BCOPY(reqopts, realtime_info->v6.reqopts, MAXNAMELEN);
        realtime_info->v6.reqopts_len = reqopts_len;
    }
    if (vendorname_len) {
        BCOPY(vendorname, realtime_info->v6.vendorname, MAXNAMELEN);
        realtime_info->v6.vendorname_len = vendorname_len;
    }
    if (clientidentifier_len) {
        BCOPY(clientidentifier, realtime_info->v6.clientidentifier, MAXNAMELEN);
        realtime_info->v6.clientidentifier_len = clientidentifier_len;
    }
    if (userclass_len) {
        BCOPY(userclass, realtime_info->v6.userclass, MAXNAMELEN);
        realtime_info->v6.userclass_len = userclass_len;
    }
...
PRIVATE int packet_process6(packet_process_t *packet_process, trash_queue_t *pRecycleTrash)
{
    ...
    if (packet_deepin_parse6(packet_process, pRecycleTrash)) {
        x_log_debug("报文解析失败ID:%d", dhcpd_server->nID);
        return -1;
    }

    // 黑/白名单匹配
    if (dhcpd_server_match_macaddr(dhcpd_server, packet_process->macaddr)) {
        x_log_debug("黑/白名单匹配失败ID:%d", dhcpd_server->nID);
        return -1;
    }
#ifdef CLIB_DEBUG
