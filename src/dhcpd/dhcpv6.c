...
PRIVATE struct vdhcpd_assignment *dhcpv6_lease(packet_process_t *packet_process, enum dhcpv6_msg msgcode)
{
    ...
    if (staticlease || msgcode == DHCPV6_MSG_RELEASE || msgcode == DHCPV6_MSG_DECLINE) {
        a->leasetime = 0;
    ...
    if (ENABLE_IPV6_SLAAC(dhcpd_server) && (reqmsg == DHCPV6_MSG_INFORMATION_REQUEST)) {
        for (u32 i = 1; i < realtime_info->v6.reqopts_len; i += 2) {
            if (realtime_info->v6.reqopts[i] == DHCPV6_OPT_DNS_SERVERS)
                ...
                u32 leastime = htonl(dhcpd_server->SLAAC.leasetime);
                dhcpv6_put(&rep, &cookie, DHCPV6_OPT_LIFETIME, 4, &leastime);
            }
        }
    } else {
        if (realtime_info->v6.ia_pd)
            dhcpv6_put(&rep, &cookie, ntohs(ia_pd.type), ntohs(ia_pd.len), ((u8 *)&ia_pd) + 4);
        else
            dhcpv6_put(&rep, &cookie, ntohs(ia_na.type), ntohs(ia_na.len), ((u8 *)&ia_na) + 4);
        for (u32 i = 1; i < realtime_info->v6.reqopts_len; i += 2) {
            if (realtime_info->v6.reqopts[i] == DHCPV6_OPT_DNS_SERVERS)
