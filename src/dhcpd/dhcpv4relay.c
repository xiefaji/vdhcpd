...
#else
    vdm->sockfd_relay4 = create_unix_socket(VNAAS_DHCP_RELAY4_IPC_DGRAM_SOCK, 1, 1);
#endif //
...
PRIVATE int packet_deepin_parse(packet_process_t *packet_process)
{
    ...
    if (request->payload_len < offsetof(struct dhcpv4_message, options) + 4 || rep->op != DHCPV4_BOOTREPLY || rep->hlen != ETH_ALEN)
        return -1;
