...
static int do_receive_recvmmsg(int fd, struct mmsghdr *msgvec, unsigned int vlen, unsigned int flags)
{
    struct timespec tv;
    memset(&tv, 0, sizeof(struct timespec));
    tv.tv_sec = 1;
    return recvmmsg(fd, msgvec, vlen, flags, &tv);
}

static int do_receive_recvmsg(int fd, struct mmsghdr *msgvec, unsigned int vlen, unsigned int flags)
{
    unsigned int i;
	int tmp;
 ...
 */
int receive_prepare(struct receive *r, int custom_addr_len, int custom_control_len)
{
...
int receive(int fd, struct receive *r, unsigned int n_packets, unsigned int flags)
{
	int tmp;
...
void receive_free_buffer(struct receive *r)
{
    if (r->packets) free(r->packets);
    r->packets = NULL;
}
 ...
 */
int receive_allocate_buffer(struct receive *r, unsigned int flags)
{
	...
	size_to_allocate = r->n_packets * (sizeof(*r->packets) + r->addr_len + sizeof(struct iovec) + r->buffer_len + r->control_len);
	...
	i = r->n_packets;
	while (i--) {
        /* r->packets[i].msg_len  contains the number of read bytes after recvmmsg() call */
		current = &r->packets[i].msg_hdr;
